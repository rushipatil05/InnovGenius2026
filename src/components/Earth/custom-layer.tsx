import React, { useRef } from "react";
import Globe from "react-globe.gl";
import * as THREE from "three";

import globeImage from "../assets/earth-dark.jpg";

/* -------------------- Types -------------------- */

interface PointData {
  lat: number;
  lng: number;
  altitude: number;
  color: string;
}

interface CustomLayerData {
  lat: number;
  lng: number;
  altitude: number;
  size: number;
  color: string;
}

/* -------------------- Component -------------------- */

const Page: React.FC = () => {
  const globeEl = useRef<any>(null); // using any since react-globe.gl has no official types

  const myData: PointData[] = [
    {
      lat: 29.953204744601763,
      lng: -90.08925929478903,
      altitude: 0.4,
      color: "#00ff33",
    },
    {
      lat: 28.621322361013092,
      lng: 77.20347613099612,
      altitude: 0.4,
      color: "#ff0000",
    },
    {
      lat: -43.1571459086602,
      lng: 172.72338919659848,
      altitude: 0.4,
      color: "#ffff00",
    },
  ];

  const customLayerData: CustomLayerData[] = [...Array(500).keys()].map(
    () => ({
      lat: (Math.random() - 1) * 360,
      lng: (Math.random() - 1) * 360,
      altitude: Math.random() * 2,
      size: Math.random() * 1,
      color: "#9999cc",
    })
  );

  return (
    <div className="cursor-move">
      <Globe
        ref={globeEl}
        globeImageUrl={globeImage}
        pointsData={myData}
        pointAltitude="altitude"
        pointColor="color"
        customLayerData={customLayerData}
        customThreeObject={(data: object) => {
          const { size, color } = data as CustomLayerData;

          return new THREE.Mesh(
            new THREE.SphereGeometry(size),
            new THREE.MeshBasicMaterial({ color })
          );
        }}
        customThreeObjectUpdate={(obj: THREE.Object3D, data: object) => {
          if (!globeEl.current) return;

          const { lat, lng, altitude } = data as CustomLayerData;

          Object.assign(
            obj.position,
            globeEl.current.getCoords(lat, lng, altitude)
          );
        }}
      />
    </div>
  );
};

export default Page;